...
function getMessageIcon(weapon, attacker)
    ...
    if path and fileExists(path) then
        ...
        if texture and isElement(texture) then
            ...
            outputDebugString("Error: No se pudo crear la textura para el ícono. Ruta: " .. tostring(path), 2)
        end
    else
        outputDebugString("Error: Archivo de ícono no encontrado. Ruta: " .. tostring(path), 2)
    end

    -- Si no se pudo crear la textura, devolver false
    return false
end
...
function getTexture(path)
    ...
    -- Verificar si la textura ya existe en la caché y es válida
    if textures[path] and isElement(textures[path]) then
        return textures[path]
    end

    -- Si no existe o no es válida, crear una nueva textura
    if fileExists(path) then
        ...
        if textures[path] and isElement(textures[path]) then
            ...
            outputDebugString("Error: No se pudo crear la textura para el ícono. Ruta: " .. tostring(path), 2)
            textures[path] = nil -- Eliminar la entrada inválida de la caché
        end
    else
        outputDebugString("Error: Archivo de ícono no encontrado. Ruta: " .. tostring(path), 2)
    end
